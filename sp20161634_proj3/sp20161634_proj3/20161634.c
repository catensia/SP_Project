...
//hash table pointer
extern struct NODE *hash[MAX_HASH];
extern int progaddr;
extern int bp_num;
//works as 1MB virtual memory 1byte * 2^20
extern unsigned char m[2<<20];
int dump_adr=0;
...
int argnum(char* cmd){
    ...
    //TWO WORDED INPUTS
    if(words==2 && trash[0]=='\0'){
        if(!strcmp(op, "opcode")){
            //ex: opcode add
            op_cmd(mne, hash, &head); return 0; 
        }
        if(!strcmp(op, "type")){
            type_filename(mne, &head); return 0;
        }
        if(!strcmp(op, "assemble")){
            int ret=assemble(mne, &head); 
            if(ret!=-1){
                sprintf(history_string, "assemble %s", mne);
                history_add(history_string, &head);
            }
            return 0;
        }
        if(!strcmp(op, "progaddr")){
            sscanf(mne, "%x", &progaddr);
            sprintf(history_string, "progaddr %s", mne);
            history_add(history_string, &head);
            return 0;
        }
        ...
        if(!strcmp(op, "bp")){
            ...
            if(!strcmp(mne, "clear")){
                ...
                sprintf(history_string, "bp clear");
                history_add(history_string, &head);
                return 0;
            }
            int new_bp;
            ...
            sprintf(history_string, "bp %X", new_bp);
            history_add(history_string, &head);
            return 0;
        }
    }
