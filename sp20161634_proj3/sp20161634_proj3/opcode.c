...
int op_format(const char* mnemonic, struct NODE* hash[]){
    ...
    while(cur!=NULL){
        if(!strcmp(cur->cmd, mnemonic)){
            if(!strcmp(cur->format, "1")) return 1;
            else if(!strcmp(cur->format, "2")) return 2;
            else if(!strcmp(cur->format, "3/4")) return 3;
        }
        cur=cur->next;
    }
    return -1;
...
int get_format(int code, struct NODE* hash[]){
    for(int i=0;i<20;i++){
        for(struct NODE* cur=hash[i];cur!=NULL;cur=cur->next){
            if(cur->num==code){
                if(!strcmp(cur->format, "1")) return 1;
                else if(!strcmp(cur->format, "2")) return 2;
                else if(!strcmp(cur->format, "3/4")) return 3;
